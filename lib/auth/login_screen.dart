...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Widget _buildGoogleSignInButton() {
    return GestureDetector(
      onTap: () async {
        final userCredential = await _auth.loginWithGoogle(context);
        if (userCredential != null) {
          log("Google Sign-In successful");
          goToHome(context); // Navigate if login is successful
        } else {
  ...
  void goToHome(BuildContext context) {
    if (mounted) {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => const HomeScreen()), // Navigates to HomeScreen
      );
    }
  }
